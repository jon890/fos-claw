...
| `/study-pack <topic>` (native) | 토픽 1개 풀 마크다운 스터디팩 → fos-study 푸시. overlap 점검 + update vs new 판단 + self-check 내장 (plan014에서 옛 `maintain-study-pack` 흡수) | ✓ | 토픽별 1회 또는 갱신 |
| `/interview-asset <topic>` (native) | 후보자 이력 중심 Q&A 질문 은행 + 마스터 플레이북 → fos-study 푸시 (plan015) | ✓ | 토픽별 1회 또는 갱신 |
...
(옛 `live-coding-dispatch`는 plan016에서 study-topic-recommender native skill로 흡수됨. `bootcamp-batch`는 plan014, `auto-question-bank`는 plan015에서 폐기 — 사용자가 직접 `claude -p "/study-pack <topic>"` 또는 `claude -p "/interview-asset <topic>"` 반복 호출하는 흐름으로 대체)
