...
- 외부 공유용 (블로그·인터뷰 자산): `sources/fos-study/` (git 동기 외부 저장소). study-pack / question-bank / master가 즉시 commit + push.
...
plan005(ADR-017)로 wire-up되는 항목 — `live-coding-dispatch` / `auto-question-bank` 명령으로 등재. (옛 `bootcamp-batch`는 plan014에서 폐기 — 사용자가 직접 `claude -p "/study-pack <topic>"` 반복 호출하는 흐름으로 대체)
