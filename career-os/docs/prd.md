...
| `/candidate-baseline-suggester` (native) | fos-study 학습 이력 기반으로 candidate-profile.md · baseline-core-files.json · prd.md '약점·강점' · study-progress.json weak_spots를 Append + 주석 마킹으로 갱신. audit trail → `data/runtime/profile-refresh-suggestions/YYYY-MM-DD/` (ADR-028, plan020) | 없음 | study-pack 5회 이상 누적 후 / 면접 시즌 시작 시. 최소 2주 1회 |
